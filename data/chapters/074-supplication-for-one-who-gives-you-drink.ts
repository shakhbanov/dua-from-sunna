import { ChapterData } from '../../types';

export const CHAPTER_074: ChapterData = {
    id: 74,
    title: { ru: "Мольба, с которой человеку желательно обратиться к Аллаху за того, кто предложил ему напиться или хотел сделать это", en: "Supplication for one who offers you drink" },
    duas: [
      {
        id: "74-1",
        audioUrl: "https://s3.twcstorage.ru/44a93b74-shakhbanov/hisn-al-muslim/183.wav",
        fullTranslation: {
          ru: "О Аллах! Накорми того, кто накормил меня, и напои того, кто напоил меня.",
          en: "O Allah, feed the one who fed me, and give drink to the one who gave me drink."
        },
        sync: [
          { text: "اللَّهُمَّ", trans: { ru: "О Аллах", en: "O Allah" }, start: 0, end: 0 },
          { text: "أَطْعِمْ", trans: { ru: "накорми", en: "feed" }, start: 0, end: 0 },
          { text: "مَنْ", trans: { ru: "того, кто", en: "the one who" }, start: 0, end: 0 },
          { text: "أَطْعَمَنِي", trans: { ru: "накормил меня", en: "fed me" }, start: 0, end: 0 },
          { text: "وَاسْقِ", trans: { ru: "и напои", en: "and give drink to" }, start: 0, end: 0 },
          { text: "مَنْ", trans: { ru: "того, кто", en: "the one who" }, start: 0, end: 0 },
          { text: "سَقَانِي", trans: { ru: "напоил меня", en: "gave me drink" }, start: 0, end: 0 }
        ],
        source: { ru: "Муслим 5362", en: "Muslim 5362" }
      }
    ]
  };
