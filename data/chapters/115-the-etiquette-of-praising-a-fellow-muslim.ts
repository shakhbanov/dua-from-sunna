import { ChapterData } from '../../types';

export const CHAPTER_115: ChapterData = {
    id: 115,
    title: { ru: "Что желательно говорить мусульманину, если он хочет похвалить другого мусульманина", en: "The etiquette of praising a fellow Muslim" },
    duas: [
      {
        id: "115-1",
        audioUrl: "https://s3.twcstorage.ru/44a93b74-shakhbanov/dua.shakhbanov.org/dua-from-sunna/226.mp3",
        narration: {
          ru: "Передают, что, когда в его присутствии один человек принялся хвалить другого, Посланник Аллаха ﷺ воскликнул: «Горе тебе, ты перерезал шею своему товарищу!»¹ — а затем сказал: «Если кому-либо из вас обязательно надо похвалить своего брата, пусть скажет:»",
          en: "When a man began to praise another in his presence, the Messenger of Allah ﷺ exclaimed: \"Woe to you, you have cut your companion's neck!\"¹ Then he said: \"If any of you must praise his brother, let him say:\""
        },
        fullTranslation: {
          ru: "Я думаю, что такой-то [таков-то], но Аллах [лучше] знает его, и никого не восхваляю я пред Аллахом² — я думаю, что такой-то таков-то и таков-то, — если тебе действительно известно это о нём.",
          en: "I reckon so-and-so [is such], but Allah knows him best; I do not extol anyone before Allah² — I reckon him to be such and such — if indeed you know that of him."
        },
        sync: [
          { text: "أَحْسِبُ", trans: { ru: "Я думаю", en: "I reckon" }, start: 0.32, end: 0.75 },
          { text: "فُلَانًا", trans: { ru: "что такой-то", en: "so-and-so" }, start: 0.75, end: 1.98 },
          { text: "وَاللَّهُ", trans: { ru: "но Аллах", en: "but Allah" }, start: 2.63, end: 3.3 },
          { text: "حَسِيبُهُ", trans: { ru: "знает его", en: "is his Reckoner" }, start: 3.3, end: 4.4 },
          { text: "وَلَا", trans: { ru: "и не", en: "and not" }, start: 5.81, end: 5.94 },
          { text: "أُزَكِّي", trans: { ru: "восхваляю я", en: "do I extol" }, start: 5.94, end: 6.65 },
          { text: "عَلَى", trans: { ru: "пред", en: "before" }, start: 6.86, end: 7.3 },
          { text: "اللَّهِ", trans: { ru: "Аллахом", en: "Allah" }, start: 7.3, end: 7.82 },
          { text: "أَحَدًا", trans: { ru: "никого", en: "anyone" }, start: 7.82, end: 8.96 },
          { text: "أَحْسِبُهُ", trans: { ru: "я думаю о нём", en: "I reckon him" }, start: 10.49, end: 11.01 },
          { text: "كَذَا", trans: { ru: "таковым", en: "(to be) such" }, start: 11.01, end: 11.71 },
          { text: "وَكَذَا", trans: { ru: "и таковым", en: "and such" }, start: 11.71, end: 12.58 },
          { text: "إِنْ", trans: { ru: "если", en: "if" }, start: 13.59, end: 13.85 },
          { text: "كَانَ", trans: { ru: "действительно", en: "he indeed" }, start: 13.95, end: 14.04 },
          { text: "يَعْلَمُ", trans: { ru: "известно", en: "knows" }, start: 14.04, end: 14.9 },
          { text: "ذَاكَ", trans: { ru: "это", en: "that" }, start: 14.9, end: 15.46 },
          { text: "مِنْهُ", trans: { ru: "о нём", en: "of him" }, start: 15.46, end: 16.08 }
        ],
        note: {
          ru: "¹ Имеется в виду, что чрезмерные восхваления могут погубить того, кого хвалят, заставив его возгордиться. ² То есть не ручаюсь за его положение перед Аллахом.",
          en: "¹ Meaning that excessive praise can destroy the one praised by filling him with pride. ² That is, I do not vouch for his state before Allah."
        },
        source: { ru: "аль-Бухари 2662; Муслим 3000", en: "al-Bukhari 2662; Muslim 3000" }
      }
    ]
  };
