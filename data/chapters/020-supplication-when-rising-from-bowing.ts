import { ChapterData } from '../../types';

export const CHAPTER_020: ChapterData = {
    id: 20,
    title: { ru: "Слова поминания Аллаха, которые произносятся при выпрямлении после поясного поклона", en: "Supplication when rising from bowing" },
    duas: [
      {
        id: "20-1",
        audioUrl: "https://s3.twcstorage.ru/44a93b74-shakhbanov/hisn-al-muslim/38.wav",
        fullTranslation: {
          ru: "Да услышит Аллах тех (или: того), кто воздал Ему хвалу.",
          en: "May Allah hear the one who praises Him."
        },
        sync: [
          { text: "سَمِعَ", trans: { ru: "Да услышит", en: "May hear" }, start: 0, end: 0 },
          { text: "اللَّهُ", trans: { ru: "Аллах", en: "Allah" }, start: 0, end: 0 },
          { text: "لِمَنْ", trans: { ru: "того, кто", en: "the one who" }, start: 0, end: 0 },
          { text: "حَمِدَهُ", trans: { ru: "воздал Ему хвалу", en: "praises Him" }, start: 0, end: 0 }
        ],
        note: {
          ru: "То есть: да примет Аллах эту хвалу и воздаст за неё молящемуся.",
          en: "Meaning: may Allah accept this praise and reward the one who offers it."
        },
        source: { ru: "аль-Бухари 795", en: "al-Bukhari 795" }
      },
      {
        id: "20-2",
        audioUrl: "https://s3.twcstorage.ru/44a93b74-shakhbanov/hisn-al-muslim/39.wav",
        fullTranslation: {
          ru: "Господь наш, хвала же Тебе, хвала многая, благая и благословенная!",
          en: "Our Lord, to You is the praise — much, good, and blessed praise."
        },
        sync: [
          { text: "رَبَّنَا", trans: { ru: "Господь наш", en: "Our Lord" }, start: 0, end: 0 },
          { text: "وَلَكَ", trans: { ru: "и Тебе", en: "and to You" }, start: 0, end: 0 },
          { text: "الْحَمْدُ", trans: { ru: "хвала", en: "is the praise" }, start: 0, end: 0 },
          { text: "حَمْدًا", trans: { ru: "хвала", en: "praise" }, start: 0, end: 0 },
          { text: "كَثِيرًا", trans: { ru: "многая", en: "abundant" }, start: 0, end: 0 },
          { text: "طَيِّبًا", trans: { ru: "благая", en: "good" }, start: 0, end: 0 },
          { text: "مُبَارَكًا", trans: { ru: "благословенная", en: "blessed" }, start: 0, end: 0 },
          { text: "فِيهِ", trans: { ru: "—", en: "in it" }, start: 0, end: 0 }
        ],
        source: { ru: "аль-Бухари 799", en: "al-Bukhari 799" }
      },
      {
        id: "20-3",
        audioUrl: "https://s3.twcstorage.ru/44a93b74-shakhbanov/hisn-al-muslim/40.wav",
        fullTranslation: {
          ru: "Господь наш! Хвала Тебе, и пусть [эта хвала] наполнит собой небеса, землю, то, что находится между ними, и всё, что ещё Тебе будет угодно. Ты [больше всех] достоин восхваления и прославления, лучшим же из сказанного рабом [Твоим], — а все мы — Твои рабы, — [являются слова] «О Аллах! Никто не лишит того, что Ты даровал, и никто не дарует того, чего Ты лишил, и бесполезным пред Тобой окажется богатство обладающего богатством».",
          en: "Our Lord, to You is the praise — filling the heavens, filling the earth, filling whatever lies between them, and filling whatever else You will. You are most deserving of praise and glory. The truest word a servant has spoken — and we are all Your servants — is: \"O Allah, none can withhold what You have given, and none can give what You have withheld; and the wealth of the wealthy will not avail against You.\""
        },
        sync: [
          { text: "رَبَّنَا", trans: { ru: "Господь наш", en: "Our Lord" }, start: 0, end: 0 },
          { text: "لَكَ", trans: { ru: "Тебе", en: "to You" }, start: 0, end: 0 },
          { text: "الْحَمْدُ", trans: { ru: "хвала", en: "is the praise" }, start: 0, end: 0 },
          { text: "مِلْءَ", trans: { ru: "наполнив", en: "filling" }, start: 0, end: 0 },
          { text: "السَّمَاوَاتِ", trans: { ru: "небеса", en: "the heavens" }, start: 0, end: 0 },
          { text: "وَمِلْءَ", trans: { ru: "и наполнив", en: "and filling" }, start: 0, end: 0 },
          { text: "الْأَرْضِ", trans: { ru: "землю", en: "the earth" }, start: 0, end: 0 },
          { text: "وَمَا", trans: { ru: "и то, что", en: "and what" }, start: 0, end: 0 },
          { text: "بَيْنَهُمَا", trans: { ru: "между ними", en: "is between them" }, start: 0, end: 0 },
          { text: "وَمِلْءَ", trans: { ru: "и наполнив", en: "and filling" }, start: 0, end: 0 },
          { text: "مَا", trans: { ru: "то, что", en: "whatever" }, start: 0, end: 0 },
          { text: "شِئْتَ", trans: { ru: "Ты пожелаешь", en: "You will" }, start: 0, end: 0 },
          { text: "مِنْ", trans: { ru: "из", en: "of" }, start: 0, end: 0 },
          { text: "شَيْءٍ", trans: { ru: "чего-либо", en: "anything" }, start: 0, end: 0 },
          { text: "بَعْدُ", trans: { ru: "ещё", en: "afterward" }, start: 0, end: 0 },
          { text: "أَهْلَ", trans: { ru: "Достойный", en: "[You are] worthy of" }, start: 0, end: 0 },
          { text: "الثَّنَاءِ", trans: { ru: "восхваления", en: "praise" }, start: 0, end: 0 },
          { text: "وَالْمَجْدِ", trans: { ru: "и прославления", en: "and glory" }, start: 0, end: 0 },
          { text: "أَحَقُّ", trans: { ru: "наиболее достойное", en: "the truest" }, start: 0, end: 0 },
          { text: "مَا", trans: { ru: "из того, что", en: "of what" }, start: 0, end: 0 },
          { text: "قَالَ", trans: { ru: "сказал", en: "has been said by" }, start: 0, end: 0 },
          { text: "الْعَبْدُ", trans: { ru: "раб", en: "the servant" }, start: 0, end: 0 },
          { text: "وَكُلُّنَا", trans: { ru: "а все мы", en: "and all of us" }, start: 0, end: 0 },
          { text: "لَكَ", trans: { ru: "Тебе", en: "to You" }, start: 0, end: 0 },
          { text: "عَبْدٌ", trans: { ru: "рабы", en: "are servants" }, start: 0, end: 0 },
          { text: "اللَّهُمَّ", trans: { ru: "О Аллах", en: "O Allah" }, start: 0, end: 0 },
          { text: "لَا", trans: { ru: "нет", en: "none" }, start: 0, end: 0 },
          { text: "مَانِعَ", trans: { ru: "лишающего", en: "can withhold" }, start: 0, end: 0 },
          { text: "لِمَا", trans: { ru: "того, что", en: "what" }, start: 0, end: 0 },
          { text: "أَعْطَيْتَ", trans: { ru: "Ты даровал", en: "You have given" }, start: 0, end: 0 },
          { text: "وَلَا", trans: { ru: "и нет", en: "and none" }, start: 0, end: 0 },
          { text: "مُعْطِيَ", trans: { ru: "дарующего", en: "can give" }, start: 0, end: 0 },
          { text: "لِمَا", trans: { ru: "того, что", en: "what" }, start: 0, end: 0 },
          { text: "مَنَعْتَ", trans: { ru: "Ты лишил", en: "You have withheld" }, start: 0, end: 0 },
          { text: "وَلَا", trans: { ru: "и не", en: "and not" }, start: 0, end: 0 },
          { text: "يَنْفَعُ", trans: { ru: "приносит пользы", en: "avails" }, start: 0, end: 0 },
          { text: "ذَا", trans: { ru: "обладателю", en: "the possessor of" }, start: 0, end: 0 },
          { text: "الْجَدِّ", trans: { ru: "богатства", en: "wealth" }, start: 0, end: 0 },
          { text: "مِنْكَ", trans: { ru: "пред Тобой", en: "against You" }, start: 0, end: 0 },
          { text: "الْجَدُّ", trans: { ru: "богатство", en: "his wealth" }, start: 0, end: 0 }
        ],
        note: {
          ru: "Иначе говоря, богатство не спасёт богатого от гнева Аллаха.",
          en: "Meaning: wealth will not save the wealthy from Allah's wrath."
        },
        source: { ru: "Муслим 1071, 1072", en: "Muslim 1071, 1072" }
      }
    ]
  };
